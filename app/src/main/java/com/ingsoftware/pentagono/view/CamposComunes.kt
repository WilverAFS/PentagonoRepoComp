...
object Validaciones {
    ...
    val curpRegex     = Regex("^[A-Z][AEIOU][A-Z]{2}\\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\\d|3[01])[HM][A-Z]{2}[B-DF-HJ-NP-TV-Z]{3}[0-9A-Z]\\d$")
...
fun CampoTelefono(value: String, onValueChange: (String) -> Unit, obligatorio: Boolean = true) {
    ...
    PentagonoTextField(value, onValueChange, "Teléfono (10 dígitos)", isError, KeyboardType.Phone)
    if (isError) {
...
fun CampoCorreo(value: String, onValueChange: (String) -> Unit, obligatorio: Boolean = true) {
    ...
    PentagonoTextField(value, onValueChange, "Correo electrónico", isError, KeyboardType.Email)
    if (isError) {
        if (obligatorio && value.isBlank()) ErrorText("Campo obligatorio")
...
fun CampoNombre(value: String, onValueChange: (String) -> Unit, label: String = "Nombre", obligatorio: Boolean = true) {
    ...
    PentagonoTextField(value, onValueChange, label, isError, KeyboardType.Text)
    if (isError) {
        if (obligatorio && value.isBlank()) ErrorText("Campo obligatorio")
...
fun CampoNumero(value: String, onValueChange: (String) -> Unit, label: String, obligatorio: Boolean = true) {
    ...
    PentagonoTextField(value, onValueChange, label, isError, KeyboardType.Number)
    if (isError) {
        if (obligatorio && value.isBlank()) ErrorText("Campo obligatorio")
...
fun CampoCurp(value: String, onValueChange: (String) -> Unit, obligatorio: Boolean = true) {
    ...
    PentagonoTextField(value, { onValueChange(it.uppercase()) }, "CURP (18 caracteres)", isError, KeyboardType.Text)
    if (isError) {
        if (obligatorio && value.isBlank()) ErrorText("Campo obligatorio")
